...
/*页表相关*/
#define PageTblNumAddr \
 0x500 // 页表数量放在这个位置,必须与load.inc中一致					add by
       // visual 2016.5.11
#define KernelPageTblAddr \
 0x200000 // 内核页表物理地址，必须与load.inc中一致			add by visual
          // 2016.5.17
/*线性地址描述*/            // edit by visual 2016.5.25
#define KernelSize 0x800000 // 内核的大小//add by visual 2016.5.10
#define K_PHY2LIN(x) \
 ((x) + 0xC0000000) // 内核中物理地址转线性地址		//add by visual 2016.5.10
...
#define TextLinLimitMAX \
 (TextLinBase + 0x20000000) // 大小：512M，这是参考值，具体以elf描述为准，
#define DataLinBase \
 TextLinLimitMAX // 进程数据的起始地址，这是参考值，具体以elf描述为准
...
#define SharePageBase \
 VpageLinLimitMAX // 共享页线性地址，执行fork\pthread的时候用,共享页必须4K对齐
...
/***************目前线性地址布局*****************************		edit by
 *visual 2016.5.25 进程代码		0 ~ 512M ,限制大小为512M 进程数据		512M ~
 *1G，限制大小为512M 进程保留内存（以后可能存放虚页表和其他一些信息） 1G ~
 *1G+128M，限制大小为128M,共享页放在这个位置 进程堆			1G+128M ~
 *2G+128M，限制大小为1G 进程栈			2G+128M ~ 3G-4K,限制大小为 1G-128M-4K
