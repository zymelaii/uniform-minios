...
int sys_fork() {
    ...
    if (0 == p_child) {
        ...
        init_page_pte(
            p_child->task.pid); // 这里面已经填写了该进程的cr3寄存器变量
...
static int fork_mem_cpy(u32 ppid, u32 pid) {
    ...
    //             ppid,
    //             addr_lin), //
    //             物理地址，为MAX_UNSIGNED_INT时，由该函数自动分配物理内存
    ...
    for (addr_lin = p_proc_current->task.memmap.heap_lin_base;
         addr_lin < p_proc_current->task.memmap.heap_lin_limit;
         addr_lin += num_4K) {
        lin_mapping_phy(
            SharePageBase,
            0,
    ...
    for (addr_lin = p_proc_current->task.memmap.arg_lin_base;
         addr_lin < p_proc_current->task.memmap.arg_lin_limit;
         addr_lin += num_4K) {
        lin_mapping_phy(
            SharePageBase,
            0,
...
static int fork_pcb_cpy(PROCESS* p_child) {
    ...
    char* p_reg; // point to a register in the new kernel stack, added by xw,
                 // 17/12/11
    //	char* esp_save_int, esp_save_context;	//It's not what you want! damn
    // it.
    char *esp_save_int,
    ...
    //	p_child->task.esp_save_context = (char*)(p_child + 1) - P_STACKTOP - 4 *
    // 6;
    memcpy(
...
static int fork_update_info(PROCESS* p_child) {
    ...
    p_proc_current->task.info
        .child_process[p_proc_current->task.info.child_p_num - 1] =
    ...
    /************更新子进程的info***************/
    p_child->task.info.type =
        p_proc_current->task.info.type; // 当前进程属性跟父进程一样
