...
u32 sys_exec(char* path) {
    ...
    /*************释放进程内存****************/
    // 目前还没有实现
    // 思路是：数据、代码根据text_info和data_info属性决定释放深度，其余内存段可以完全释放
    ...
    *((u32*)(p_reg + ESPREG - P_STACKTOP)) =
    ...
    for (addr_lin = p_proc_current->task.memmap.stack_lin_base;
        ...
        err_temp = lin_mapping_phy(
            addr_lin,         // 线性地址						//add by visual 2016.5.9
            MAX_UNSIGNED_INT, // 物理地址						//edit by visual
                              // 2016.5.19
            p_proc_current->task
...
static int exec_pcb_init(char* path) {
    ...
    strncpy(
        ...
        path,
        sizeof(p_proc_current->task.p_name) - 1); // 名称
    ...
    p_proc_current->task.regs.cs =
        ((8 * 0) & SA_RPL_MASK & SA_TI_MASK) | SA_TIL | RPL_USER;
    p_proc_current->task.regs.ds =
        ((8 * 1) & SA_RPL_MASK & SA_TI_MASK) | SA_TIL | RPL_USER;
    p_proc_current->task.regs.es =
        ((8 * 1) & SA_RPL_MASK & SA_TI_MASK) | SA_TIL | RPL_USER;
    p_proc_current->task.regs.fs =
        ((8 * 1) & SA_RPL_MASK & SA_TI_MASK) | SA_TIL | RPL_USER;
    p_proc_current->task.regs.ss =
        ((8 * 1) & SA_RPL_MASK & SA_TI_MASK) | SA_TIL | RPL_USER;
    p_proc_current->task.regs.gs =
