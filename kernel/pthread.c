...
int do_pthread(void *entry) {
    ...
    if (0 == p_child) {
        ...
        PROCESS *p_parent;
        if (p_proc_current->task.info.type == TYPE_THREAD) {          // 线程
        ...
        *((u32 *)(p_reg + EIPREG - P_STACKTOP)) =
            p_child->task.regs.eip; // added by xw, 17/12/11
...
static int pthread_update_info(PROCESS *p_child, PROCESS *p_parent) {
    ...
    if (p_parent != p_proc_current) { // 只有在线程创建线程的时候才会执行
        ...
        p_parent->task.info.child_thread[p_parent->task.info.child_t_num - 1] =
    ...
    p_proc_current->task.info
        .child_thread[p_proc_current->task.info.child_t_num - 1] =
        p_child->task.pid; // 子线程列表
...
static int pthread_stack_init(PROCESS *p_child, PROCESS *p_parent) {
    ...
    p_child->task.memmap.stack_lin_limit =
        p_parent->task.memmap.stack_child_limit;       // 子线程的栈界
    p_parent->task.memmap.stack_child_limit += 0x4000; // 分配16K
    p_child->task.memmap.stack_lin_base =
        p_parent->task.memmap.stack_child_limit - num_4B; // 子线程的基址
    ...
    *((u32 *)(p_reg + ESPREG - P_STACKTOP)) =
        p_child->task.regs.esp; // added by xw, 17/12/11
 ...
 *************************************************************/
static int pthread_heap_init(PROCESS *p_child, PROCESS *p_parent) {
    p_child->task.memmap.heap_lin_base =
        (u32) & (p_parent->task.memmap.heap_lin_base);
    p_child->task.memmap.heap_lin_limit =
        (u32) & (p_parent->task.memmap.heap_lin_limit);
    return 0;
}
