 ...
 *======================================================================*/
u32 init_page_pte(u32 pid) { // 页表初始化函数
    ...
    if (pde_addr_phy_temp < 0
        || (pde_addr_phy_temp & 0x3FF) != 0) // add by visual 2016.5.9
    {
    ...
    for (AddrLin = KernelLinBase, phy_addr = 0;
         AddrLin < KernelLinBase + KernelSize;
         AddrLin += num_4K,
        phy_addr +=
         num_4K) { // 只初始化内核部分，3G后的线性地址映射到物理地址开始处
        err_temp = lin_mapping_phy(
            ...
            PG_P | PG_USU | PG_RWW, // 页目录的属性位（用户权限）
                                    // //edit by visual 2016.5.26
            PG_P | PG_USS | PG_RWW); // 页表的属性位（系统权限）
                                     // //edit by visual 2016.5.17
        if (err_temp != 0) {
...
void page_fault_handler(
    u32 vec_no,   // 异常编号，此时应该是14，代表缺页异常
    u32 err_code, // 错误码
...
{                 // 缺页中断处理函数
    ...
    if (kernel_initial == 1) {
        kprintf(
            "\n[Page Fault] eip=%08x eflags=0x%x cs=0x%x err_code=%d cr2=0x%x",
            eip,
            eflags,
            cs,
            err_code,
    ...
    if (cr2 == cr2_save) {
        ...
        if (cr2_count == 5) {
            vga_set_disppos(0);
            kprintf(
                "\n[Page Fault] eip=%08x eflags=0x%x cs=0x%x err_code=%d "
                "cr2=0x%x cr3=0x%x pde=0x%x pte=0x%x",
                eip,
                eflags,
                cs,
                err_code,
 ...
 *======================================================================*/
inline u32 get_pde_index(u32 AddrLin) { // 由 线性地址 得到 页目录项编号
 ...
 *======================================================================*/
inline u32 get_pte_index(u32 AddrLin) { // 由 线性地址 得到 页表项编号
    return (((AddrLin)&0x003FFFFF) >> 12); // 中间10位A21~A12,0x3FFFFF = 0000
...
inline u32 get_pte_phy_addr(
    u32 pid,     // 页目录物理地址		//edit by visual 2016.5.19
    u32 AddrLin) // 线性地址
{                // 获取该线性地址所属页表的物理地址
    u32 PageDirPhyAddr = get_pde_phy_addr(pid); // add by visual 2016.5.19
    return (*((u32 *)K_PHY2LIN(PageDirPhyAddr) + get_pde_index(AddrLin)))
         & 0xFFFFF000; // 先找到该进程页目录首地址，然后计算出该线性地址对应的页目录项，再访问,最后注意4k对齐
}
...
inline u32 get_page_phy_addr(
    u32 pid,     // 页表物理地址				//edit by visual 2016.5.19
    u32 AddrLin) // 线性地址
{                // 获取该线性地址对应的物理页物理地址
    u32 PageTblPhyAddr =
        get_pte_phy_addr(pid, AddrLin); // add by visual 2016.5.19
    return (*((u32 *)K_PHY2LIN(PageTblPhyAddr) + get_pte_index(AddrLin)))
         & 0xFFFFF000;
}
...
{                       // 判断 有没有 页表
    if ((0x00000001
        ...
        == 0) // 先找到该进程页目录,然后计算出该线性地址对应的页目录项,访问并判断其是否存在
    {         // 标志位为0，不存在
        return 0;
    } else {
        return 1;
...
{                       // 判断 该线性地址 有没有 对应的 物理页
    if ((0x00000001
         & (*((u32 *)K_PHY2LIN(PageTblPhyAddr) + get_pte_index(AddrLin))))
        == 0) { // 标志位为0，不存在
        return 0;
    } else {
        return 1;
...
void write_page_pte(
    ...
    u32 Attribute) // 属性
{                  // 填写页目录，会添加属性
    (*((u32 *)K_PHY2LIN(TblPhyAddr) + get_pte_index(AddrLin))) =
        (PhyAddr & 0xFFFFF000) | Attribute;
    // 页表起始地址+一项的大小*所属的项
}
