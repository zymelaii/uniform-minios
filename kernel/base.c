...
void ReadSector(BYTE *buf, DWORD sectorIndex) {
    ...
    RD_SECT_SCHED_FAT(
        fat32_dev, buf, sectorIndex); // modified by mingxuan 2020-10-27
}
...
void DeleteAllRecord(DWORD startCluster) {
    ...
    buf  = (PBYTE)kmalloc(Bytes_Per_Sector * sizeof(BYTE));
    ...
    do {
        ...
        if (curSectorIndex != preSectorIndex) {
            if (preSectorIndex != 0) { WriteSector(buf, preSectorIndex); }
        ...
        if (record.filename[0] != (BYTE)0xE5 && record.filename[0] != 0) {
            ...
            start = (DWORD)(record.highClusterNum << 16)
                  + (DWORD)record.lowClusterNum;
...
void GetFileOffset(
    PFile pfile, PDWORD sectorIndex, PDWORD off_in_sector, PUINT isLastSector) {
    DWORD curSectorIndex = 0, nextSectorIndex = 0, sectorNum = 0,
          totalSectors = 0, counter = 0;
    ...
    curSectorIndex = Reserved_Sector + 2 * Sectors_Per_FAT
                   + (pfile->start - 2) * Sectors_Per_Cluster;
    do {
        counter++;
...
void GetNextSector(
    PFile  pfile,
    DWORD  curSectorIndex,
    PDWORD nextSectorIndex,
    PUINT  isLastSector) {
    DWORD temp            = 0;
    ...
    off_in_cluster =
        (BYTE)(temp % Sectors_Per_Cluster); // 此扇区是所在簇的的几个扇区，从零开始
    ...
    if (nextClusterIndex == FAT_END) // 此扇区所在簇是该文件的最后一个簇
    {
        if (pfile->flag == R) {
            ...
            if (temp == 0) // 此文件实际占用了整数个簇
            {
                ...
                if (temp % Bytes_Per_Sector == 0) {
                    ...
                    last =
                        temp / Bytes_Per_Sector + 1; // 最后一簇实际占用的扇区数
                }
            }
        } else {
            last = Sectors_Per_Cluster;
        ...
        if (off_in_cluster < last - 1) {
            (*nextSectorIndex) = Reserved_Sector + 2 * Sectors_Per_FAT
                               + (curClusterIndex - 2) * Sectors_Per_Cluster
                               + off_in_cluster + 1;
            if (off_in_cluster
                == last - 2) // 返回的扇区号是该文件的最后一个扇区
            {
                *isLastSector = 1;
            } else {
                *isLastSector = 0;
...
STATE ReadRecord(
    ...
    PRecord record,
    PDWORD  sectorIndex,
    PDWORD  off_in_sector) {
    CHAR  temp[256]      = {0};
    DWORD curSectorIndex = 0, curClusterIndex = parentCluster, off = 0,
          size_of_Record;
    BYTE *buf;
    UINT  last = 0;
    ...
    do {
        ...
        {
            ReadSector(buf, curSectorIndex);
            for (off = 0; off < Bytes_Per_Sector;
                 off += size_of_Record) // 目录项号循环
            {
...
STATE ClearRecord(DWORD parentCluster, PCHAR name, PDWORD startCluster) {
    ...
    state =
        ReadRecord(parentCluster, name, &record, &sectorIndex, &off_in_sector);
    if (state != OK) { return state; }
    record.filename[0] = (BYTE)0xE5;
    startClusterIndex =
        ((DWORD)record.highClusterNum << 16) + (DWORD)record.lowClusterNum;
...
STATE PathToCluster(PCHAR path, PDWORD cluster) {
    ...
    for (i = i + 1; i < len; i++) {
        if (fullpath[i] == '\\' || i == len - 1) {
            if (i == len - 1) { name[j] = fullpath[i]; }
            state = ReadRecord(parentCluster, name, &record, NULL, NULL);
            if (state == OK) {
                parentCluster =
                    (record.highClusterNum << 16) + record.lowClusterNum;
...
STATE FindSpaceInDir(
    DWORD  parentClusterIndex,
    PCHAR  name,
    PDWORD sectorIndex,
    PDWORD off_in_sector) {
    PBYTE  buf = NULL;
    ...
    do // 簇号循环
    {
        ...
        {
            ReadSector(buf, curSectorIndex);
            for (offset = 0; offset < Bytes_Per_Sector;
                 offset += sizeof(Record)) // 目录项号循环
            {
...
STATE WriteFAT(DWORD totalclusters, PDWORD clusters) {
    ...
    for (i = 0; i < totalclusters; i++) {
        ...
        if (curSectorIndex != preSectorIndex) // 两个簇号不在同一个扇区。
        {
            if (preSectorIndex != 0) {
                WriteSector(
                    buf, preSectorIndex); // 先把上一个扇区的内容写入FAT1中
            }
            ReadSector(buf, curSectorIndex);
            preSectorIndex = curSectorIndex;
        }
...
STATE AllotClustersForEmptyFile(PFile pfile, DWORD size) {
    ...
    clusters = (PDWORD)kmalloc(n * sizeof(DWORD));
...
STATE AddCluster(
    ...
    DWORD num) // cluster表示该文件或目录所在目录的簇,num表示增加几个簇
{
    PDWORD clusters        = NULL;
    DWORD  curClusterIndex = startClusterIndex, nextClusterIndex = 0;
    // DWORD bytes_per_sector=Sectors_Per_FAT*Bytes_Per_Sector;
    STATE state;
    clusters = (PDWORD)kmalloc((num + 1) * sizeof(DWORD));
    if (clusters == NULL) { return SYSERROR; }
...
STATE NeedMoreCluster(PFile pfile, DWORD size, PDWORD number) {
    ...
    if (size > clusterNum * bytes_per_cluster
                   - pfile->off) // 空间不足需要追加更多的簇
    {
        if ((size - (clusterNum * bytes_per_cluster - pfile->off))
                % bytes_per_cluster
            == 0) {
            n = (size - (clusterNum * bytes_per_cluster - pfile->off))
              / bytes_per_cluster;
        } else {
            n = (size - (clusterNum * bytes_per_cluster - pfile->off))
                  / bytes_per_cluster
...
void ClearClusters(DWORD cluster) {
    ...
    first = Reserved_Sector + 2 * Sectors_Per_FAT
