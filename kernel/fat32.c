...
STATE CreateDir(const char *dirname) {
    ...
    CHAR   name[256]     = {0};
    CHAR   parent[256]   = {0};
    DWORD  parentCluster = 0, startCluster = 0, sectorIndex = 0,
          off_in_sector = 0;
    STATE state;
    ...
    sectorIndex = Reserved_Sector + 2 * Sectors_Per_FAT
                + (startCluster - 2) * Sectors_Per_Cluster;
    ...
    do {
        if (isLastSector) // 当前的扇区是该文件的最后一个扇区
        {
            if (pfile->size % Bytes_Per_Sector == 0) {
                ...
                free_in_sector = pfile->size % Bytes_Per_Sector
                               - off_in_sector; // 最后一个扇区的剩余量
            }
            tag = 1; // 置跳出标志
        } else {
        ...
        if (tag == 1) // 最后一个扇区或缓冲区装满了
        {
            ...
            GetNextSector(
                pfile, curSectorIndex, &nextSectorIndex, &isLastSector);
...
STATE WriteFile(int fd, const void *buf, int length) {
    ...
    GetFileOffset(pfile, &curSectorIndex, &off_in_sector, &isLastSector);
    free_in_sector = Bytes_Per_Sector - off_in_sector;
    while (free_in_sector
           < length - off_in_buf) // 当前扇区的空闲空间放不下本次要写入的内容
    {
        ReadSector(sector, curSectorIndex);
        memcpy(
            sector + off_in_sector, (void *)buf + off_in_buf, free_in_sector);
...
STATE CloseFile(int fd) {
    ...
    if (pfile->flag == R) {
        ...
        if (pfile->off < pfile->size) {
            ...
            if (isLastSector == 0) {
                curSectorIndex =
                    (curClusterIndex - Reserved_Sector - 2 * Sectors_Per_FAT)
                        / Sectors_Per_Cluster
                    + 2;
                GetNextCluster(curClusterIndex, &nextClusterIndex);
                if (nextClusterIndex
                    != FAT_END) // 说明当前簇不是此文件的最后一簇
                {
                    WriteFAT(
                        1, &curClusterIndex); // 把当前簇设置为此文件的最后一簇
        ...
        ReadRecord(
            parentCluster, pfile->name, &record, &sectorIndex, &off_in_sector);
...
STATE OpenFile(const char *filename, int mode) {
    ...
    state = FindSpaceInDir(
        ...
        name,
        &sectorIndex,
        &off_in_sector); // 检测文件名是否存在
    if (mode & O_CREAT)  // 如果用户使用了O_CREAT
    {
        if (state
            == NAMEEXIST) // 文件存在，使用O_CREAT是多余的，继续执行OpenFile即可
        {
    ...
    {
        if (state
            != NAMEEXIST) // 文件不存在，需要使用O_CREAT，用户没有使用，则报错并返回-1，表示路径有误
        {
    ...
    // 找一个未用的FILE
    for (i = 0; i < NR_FILE_DESC; i++)
        if (f_desc_table_fat[i].flag == 0) break;
    if (i >= NR_FILE_DESC) {
        kprintf("f_desc_table[] is full (PID:");
        kprintf("%d", proc2pid(p_proc_current));
        kprintf(")\n");
    ...
    // 以下是给File结构体赋值
    memset(
        f_desc_table_fat[i].parent,
        0,
        sizeof(f_desc_table_fat[i].parent)); // 初始化parent字段
    memset(
        f_desc_table_fat[i].name,
        0,
        sizeof(f_desc_table_fat[i].name)); // 初始化name字段
    strcpy(f_desc_table_fat[i].parent, parent);
    strcpy(f_desc_table_fat[i].name, name);
    f_desc_table_fat[i].start =
...
STATE DeleteFile(const char *filename) {
    ...
    state = IsFile(fullpath, &tag);
    if (state != OK) { return state; }
    if (tag == D) { return WRONGPATH; }
    state = PathToCluster(parent, &parentCluster);
    if (state != OK) { return state; }
    state = ClearRecord(parentCluster, name, &startCluster);
    if (state != OK) { return state; }
    if (startCluster != 0) { ClearFATs(startCluster); }
    return OK;
}
...
void init_fs_fat() {
    ...
    int i;
    for (i = 0; i < NR_FILE_DESC; ++i) { f_desc_table_fat[i].flag = 0; }
}
...
static void load_disk(int dev) {
    ...
    Position_Of_RootDir =
        (Reserved_Sector + Sectors_Per_FAT * 2) * Bytes_Per_Sector;
...
static void mkfs_fat() {
    ...
    DWORD sectors_per_FAT     = (TotalSectors * 512 - 8192) / 525312
                          + 1; // 每FAT所占扇区数，用此公式可以算出来：占4个字节
    ...
    memcpy(
        buf + 82, systemID, 8); // 系统ID，FAT32系统中一般取为“FAT32”：占8个字节
    ...
    WR_SECT_FAT(
        fat32_dev, buf, reserved_sector); // modified by mingxuan 2020-10-27
    ...
    // WR_SECT_FAT(buf, reserved_sector+2*sectors_per_FAT);	// deleted by
    // mingxuan 2020-10-27
    WR_SECT_FAT(
        fat32_dev,
        buf,
        reserved_sector
            + 2 * sectors_per_FAT); // modified by mingxuan 2020-10-27
}
