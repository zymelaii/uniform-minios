...
void MakeFullPath(PCHAR parent, PCHAR name, PCHAR fullpath) {
    int i = 0, j = 0, len = 0;
    len = strlen(parent);
    for (i = 0; i < len; i++) { fullpath[i] = parent[i]; }
    if (fullpath[i - 1] != '\\') { fullpath[i++] = '\\'; }
    len = strlen(name);
    for (j = 0; j < len; j++) { fullpath[i++] = name[j]; }
}
...
void ChangeCurrentPath(PCHAR addpath) {
    ...
    if (strcmp(addpath, "..") == 0) {
        ...
        len2 = strlen(addpath);
        for (i = 0; i < len2; i++) { cur_path[i + len1] = addpath[i]; }
    }
}
...
void GetNameFromPath(PCHAR path, PCHAR name) {
    ...
    for (i = i + 1; i < len; i++) { name[j++] = path[i]; }
}
...
void GetParentFromPath(PCHAR fullpath, PCHAR parent) {
    ...
    for (i = i - 1; i >= 0; i--) { parent[i] = fullpath[i]; }
}
...
STATE IsFullPath(PCHAR path) {
    ...
    {
        if (cur_path[i] != path[i]) { return FALSE; }
    }
    return TRUE;
}
...
void ToFullPath(PCHAR path, PCHAR fullpath) {
    ...
    if (IsFullPath(path)) {
        ...
        len = strlen(cur_path);
        for (i = 0; i < len; i++) { fullpath[i] = cur_path[i]; }
        if (cur_path[i - 1] != '\\') { fullpath[i++] = '\\'; }
        len = strlen(path);
        for (j = 0; j < len; j++) { fullpath[i++] = path[j]; }
    }
    len = strlen(fullpath);
    if (fullpath[len - 1] == '\\') { fullpath[len - 1] = 0; }
}
